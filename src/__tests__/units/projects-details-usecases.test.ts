...
const statsTestRepository = new StatsTestRepository();
...
describe("Module - Projects Details", () => {
	...
	describe("Update Project Detail", () => {
		describe("Prop Likes", () => {
			...
			it("deveria falhar caso o valor seja inválido.", async () => {
		...
		describe("Prop Views", () => {
			...
			it("deveria atualizar aumentando o valor do count", async () => {
				const { status, data, error } = await updateProjectDetail.execute({
					views: "1",
					id,
				});
			...
			it("deveria falhar caso seja informado um valor inválido", async () => {
				const test1 = await updateProjectDetail.execute({
					//@ts-ignore
					views: null,
					id,
